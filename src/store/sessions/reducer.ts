...
export default (state = defaultState, action: ActionType<typeof sessions>): SessionState => {
  switch (action.type) {
    ...
    case getType(sessions.addTrackFilter):
      const updatedTrackFilters = state.trackFilters.concat(action.payload).reduce(
        (updatedList, item) => {
    ...
    case getType(sessions.addFavorite):
      const updatedFavoriteSessions = state.favoriteSessions.concat(action.payload).reduce(
        (updatedList, item) => {
